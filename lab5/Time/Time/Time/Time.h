...
public:
	...
	//unsigned const friend operator*(unsigned number, CTime const& time);
...
unsigned const CTime::operator*(unsigned number, CTime const& time)
{
	return ((number * time.GetTimeStamp) % SECONDS_IN_A_DAY);
};
